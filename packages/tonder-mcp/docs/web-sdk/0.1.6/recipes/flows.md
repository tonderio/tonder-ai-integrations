...
Apple Pay does **not** use `pay()`. It is a mountable component: the SDK renders the button and owns the click, and the result arrives on `config.events.payment` instead of a returned promise.
...
Call `get_integration_recipe` with `flow: 'apple_pay'` for the full pattern, including availability codes, the synchronous-`payment` rule, event handling, and the `unmount()` requirement on route changes.
