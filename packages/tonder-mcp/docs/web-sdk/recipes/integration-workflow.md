...
| Presentation mode | No default; ask the user for `embedded` or `redirect` before calling `get_integration_recipe` for hosted-auth flows. The `apple_pay` flow does not use hosted authentication, so the value has no effect there and does not need to be asked |
...
For `flow: 'apple_pay'`, `presentation_mode` is not a real decision: Apple Pay never uses the hosted authentication step, so pass `embedded` without asking.
