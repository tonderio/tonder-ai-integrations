...
Use Angular environment files or the app's existing public configuration layer for deployment-specific Tonder values. Angular CLI supports environment-specific file replacements such as `environment.ts`, `environment.development.ts`, or `environment.staging.ts`. Do not commit a real merchant public API key or fixed SDK environment directly in the component.

Example shape:

```ts
export const environment = {
  tonderPublicApiKey: 'REPLACE_WITH_PUBLIC_TONDER_API_KEY',
  tonderEnvironment: 'stage' as 'sandbox' | 'stage' | 'production',
};
```

`currency` is checkout/business data and can stay in component state or merchant configuration; it does not need to be an environment variable unless the merchant app already manages it that way.
