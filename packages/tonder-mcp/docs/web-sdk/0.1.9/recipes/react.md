...
Use the host framework's public client configuration for deployment-specific Tonder values. Do not commit real merchant keys or fixed SDK environments directly in checkout components.

- Vite/React exposes client variables through `import.meta.env` when they use the `VITE_` prefix, for example `import.meta.env.VITE_TONDER_PUBLIC_API_KEY` and `import.meta.env.VITE_TONDER_ENVIRONMENT`.
- Next.js exposes browser variables through `process.env` only when they use the `NEXT_PUBLIC_` prefix, for example `process.env.NEXT_PUBLIC_TONDER_PUBLIC_API_KEY` and `process.env.NEXT_PUBLIC_TONDER_ENVIRONMENT`.
- Use the app's existing public config layer if it already has one.

`currency` is checkout/business data and can stay in application state or merchant configuration; it does not need to be an environment variable unless the merchant app already manages it that way.
